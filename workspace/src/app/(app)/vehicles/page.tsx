...
export default function VehiclesPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (imageFiles && imageFiles.length > 0) {
                console.log(`handleSubmit: Found ${imageFiles.length} files to upload.`);
                 const uploadPromises = Array.from(imageFiles).map(file => {
                    ...
                    console.log(`handleSubmit: Uploading ${file.name} to ${storageRef.fullPath}`);
                    return uploadBytes(storageRef, file).then(snapshot => getDownloadURL(snapshot.ref));
                });
                uploadedImageUrls = await Promise.all(uploadPromises);
                console.log("handleSubmit: All images uploaded successfully. URLs:", uploadedImageUrls);
            }

            if (uploadedImageUrls.length === 0 && !isEditing) {
                 toast({ variant: 'destructive', title: 'Error', description: 'Please upload at least one image.' });
            ...
            console.log("handleSubmit: Data object to be saved:", dataToSave);
            ...
            console.log("handleSubmit: Submission process finished. Resetting isSubmitting flag.");
            setIsSubmitting(false);
        }
    }
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-4">
                            ...
                            <div>
                                <Label htmlFor="category">Category</Label>
                                 <Select onValueChange={handleSelectChange('category')} value={vehicleData.category} required disabled={isSubmitting}>
                                    <SelectTrigger>
                            ...
                            <div>
                                ...
                                <Select onValueChange={handleSelectChange('status')} value={vehicleData.status} disabled={isSubmitting}>
                                    <SelectTrigger>
