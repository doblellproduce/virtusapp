...
import { Loader2, Star, MessageSquarePlus, CalendarClock, History } from 'lucide-react';
        ...
        <Card>
            <CardHeader>
                <div className="flex justify-between items-start">
                    ...
                    <Badge variant={getStatusVariant(reservation.status)} className={getStatusClass(reservation.status)}>
                        {reservation.status}
                    </Badge>
                </div>
            </CardHeader>
...
export default function ClientDashboardPage() {
    ...
    const handleReviewSubmit = async (rating: number, comment: string) => {
        ...
        try {
            ...
            // Add the new review to the local state to prevent submitting again
            ...
            <div className="space-y-6">
                <div className="flex items-center gap-2">
                 ...
                 {upcomingReservations.length > 0 ? (
                    ...
                    <p className="text-muted-foreground pl-8">No tienes reservaciones próximas o activas.</p>
                )}
            </div>
